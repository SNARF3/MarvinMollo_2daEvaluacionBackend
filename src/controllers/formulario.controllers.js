...
export const cambiarEstadoFormulario = async(req, res) => {
    try {
        ...
        // Obtener el id_cuenta del dueño del formulario
        const formulario = await formularioModel.ObtenerFormularioPorId(id_formulario);
        const id_cuenta = formulario?.id_cuenta;

        // Notificar al usuario si está conectado
        if (id_cuenta) {
            notifyUser(id_cuenta, {
                type: 'FORMULARIO_ESTADO_ACTUALIZADO',
                id_formulario,
                nuevo_estado,
                timestamp: new Date(),
            });
